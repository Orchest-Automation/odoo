...
class PublisherWarrantyContract(AbstractModel):
    ...
    def _get_message(self):
        ...
        nbr_users = Users.search_count([('active', '=', True)], limit=5)
        nbr_active_users = Users.search_count([("login_date", ">=", limit_date), ('active', '=', True)], limit=5)
        nbr_share_users = 0
        nbr_active_share_users = 0
        if "share" in Users._fields:
            nbr_share_users = Users.search_count([("share", "=", True), ('active', '=', True)], limit=10)
            nbr_active_share_users = Users.search_count([("share", "=", True), ("login_date", ">=", limit_date), ('active', '=', True)], limit=15)
        user = self.env.user
